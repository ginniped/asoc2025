...
def generate_and_save_image(title, ambientazione, trama):
    ...
    try:
        generated_images = comfy_client.generate_images(
            ...
            width=512,  # Richiesta esplicita di larghezza 512
            height=512, # Richiesta esplicita di altezza 512
            steps=15,
            cfg=3.5,
        )
...
"""
    ...
    # Salva la scena iniziale per la funzione "Ricomincia"
...
def continue_adventure():
    ...
    if choice_made == "Ricomincia la stessa avventura":
        ...
        # Recupera i dati iniziali per ricominciare
        session['history'] = []
        session['current_hp'] = INITIAL_HP
        session['current_scene_number'] = 1
    ...
    if check_death():
        return jsonify({"scene": "Sei morto! L'avventura è finita.", "choices": ["Ricomincia la stessa avventura", "Scegli una nuova avventura"], "current_hp": 0})
